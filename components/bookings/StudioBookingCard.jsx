...
const StudioBookingCard = ({ item, navigation}) => {
  ...
  const handleServiceDone = async (id) => {
    ...
    try {
      ...
      const response = await axios.put(
        `https://leen-app.com/public/api/seller/studioBookings/serviceIsDone/${id}`,
      ...
      Alert.alert('نجاح', 'تم إنهاء الخدمة بنجاح', [
        {
          text: 'حسنا',
          onPress: () => navigation.navigate('StudioBookings'),
        },
      ]);
    } catch (error) {
      setLoading(false);
  ...
  return (
     <View style={styles.cardContainer}>
       ...
       <View style={styles.cardContent}>
         ...
         <Text style={styles.cardTitle}>{item.studio_service.name}</Text>
       ...
       <View style={{ alignItems: 'center' }}>
         ...
         <TouchableOpacity
           onPress={() => navigation.navigate('ShowStudioBooking', { item })}
           style={styles.eyeIconButton}
         >
           <Icon name="eye" size={24} color="#435E58" />
         </TouchableOpacity>
         <Text style={styles.eyeButtonText}>عرض</Text>
